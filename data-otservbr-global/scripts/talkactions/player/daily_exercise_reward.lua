local exerciseCharges = 1500
local rewardStorage = 29997

local reward = TalkAction("!exercise")

function reward.onSay(player, words, param)
    if player then
        if player:getStorageValue(rewardStorage) >= os.time() then
            player:sendCancelMessage("You already collected your daily exercise reward.")
        else
            player:setStorageValue(rewardStorage, os.time() + 12 * 60 * 60)
