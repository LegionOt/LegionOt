local config = {
		bosses = {
		{'Bone Jaw', 'Concentrated Death', 'The Book of Death', 'Bone Jaw'},
        {'Bone Jaw', 'Concentrated Death', 'The Book of Death', 'Bone Jaw'},
        {'Bone Jaw', 'Concentrated Death', 'The Book of Death', 'Bone Jaw'},
        {'Bone Jaw', 'Concentrated Death', 'The Book of Death', 'Bone Jaw'}
	...
	positions = {
		-- other bosses
		Position(32752, 32718, 10),
		Position(32760, 32718, 10),
		Position(32754, 32723, 10),
        Position(32758, 32723, 10),

        Position(32756, 32717, 10),
        Position(32756, 32725, 10),
        Position(32753, 32721, 10),
        Position(32759, 32722, 10)
    ...
    timeStorage = 721, -- id of lever before pulled
...
local player_positions = {
	[1] = {fromPos = Position(32747, 32773, 10), toPos = Position(32754, 32713, 10)},
	[2] = {fromPos = Position(32748, 32773, 10), toPos = Position(32755, 32714, 10)},
	[3] = {fromPos = Position(32749, 32773, 10), toPos = Position(32756, 32713, 10)},
	[4] = {fromPos = Position(32750, 32773, 10), toPos = Position(32757, 32714, 10)},
	[5] = {fromPos = Position(32751, 32773, 10), toPos = Position(32753, 32714, 10)}
}


local configg = {
    maxPlayers = 0,
    room = {fromPos = Position(32752, 32715, 10), toPos = Position(32759, 32726, 10)},
    
}

function Ghuloshboss1xArea()
   Game.createMonster("Ghulosh", Position(32756, 32720, 10))
 
	return	
end

local Ghuloshboss1 = Action()
function Ghuloshboss1.onUse(player, item, fromPosition, target, toPosition, isHotkey)
...
if #getPlayersInArea(configg.room.fromPos, configg.room.toPos) > configg.maxPlayers then
  ...
  if player:getStorageValue(config.timeStorage) > os.time() then
	...
	addEvent(Ghuloshboss1xArea, 500)
    clearRoom(Position(32751, 32715, 10), Position(32760, 32727, 10), true)
    return true
end
	
Ghuloshboss1:uid(64092)
Ghuloshboss1:register()