...
local config = {
	...
	timeStorage = 707, -- storage que bloquea la palanca ----- para otra quest
...
local configg = {
    maxPlayers = 0,
    room = {fromPos = Position(33084, 32238, 12), toPos = Position(33090, 32252, 12)},
    
}

function Lion3xArea()
   Game.createMonster("Utua Stone Sting", Position(33086, 32244, 12))
   
    return 
end

local lion3 = MoveEvent()

function lion3.onStepIn(creature, item, position, fromPosition)
...
if #getPlayersInArea(configg.room.fromPos, configg.room.toPos) > configg.maxPlayers then
  ...
  ---------------------///// parate para agregar a otra quest-----------------------
	...
	player:teleportTo(Position(33088, 32241, 12))
  ...
  addEvent(Lion3xArea, 500)
    clearRoom(Position(33084, 32238, 12), Position(33090, 32252, 12), true)
