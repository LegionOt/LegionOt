local config = {
		bosses = {
		{'Dark Knowledge', 'Demon Blood', 'Knowledge Raider'},
		{'Dark Knowledge', 'Demon Blood', 'Knowledge Raider'},
		{'Dark Knowledge', 'Demon Blood', 'Knowledge Raider'}
    ...
    timeStorage = 720, -- id of lever before pulled
...
local player_positions = {
	[1] = {fromPos = Position(32721, 32749, 10), toPos = Position(32748, 32682, 10)},
	[2] = {fromPos = Position(32722, 32749, 10), toPos = Position(32749, 32681, 10)},
	[3] = {fromPos = Position(32723, 32749, 10), toPos = Position(32750, 32682, 10)},
	[4] = {fromPos = Position(32724, 32749, 10), toPos = Position(32751, 32681, 10)},
	[5] = {fromPos = Position(32725, 32749, 10), toPos = Position(32752, 32682, 10)}
}


local configg = {
    maxPlayers = 0,
    room = {fromPos = Position(32747, 32684, 10), toPos = Position(32755, 32693, 10)},
    
}

function LokathmorbossxArea()
   Game.createMonster("Lokathmor", Position(32751, 32689, 10))
 
	return	
end

local Lokathmorboss = Action()
function Lokathmorboss.onUse(player, item, fromPosition, target, toPosition, isHotkey)
...
if #getPlayersInArea(configg.room.fromPos, configg.room.toPos) > configg.maxPlayers then
  ...
  if player:getStorageValue(config.timeStorage) > os.time() then
	...
	addEvent(LokathmorbossxArea, 500)
    clearRoom(Position(32746, 32683, 10), Position(32755, 32694, 10), true)
    return true
end
	
Lokathmorboss:uid(64093)
Lokathmorboss:register()