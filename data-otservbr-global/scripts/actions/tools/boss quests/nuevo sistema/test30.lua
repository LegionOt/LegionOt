local config = {		
    ...
    timeStorage = 738, -- id of lever before pulled
...
local player_positions = {
    [1] = {fromPos = Position(33516, 31055, 15), toPos = Position(33545, 31016, 15)},
	[2] = {fromPos = Position(33517, 31055, 15), toPos = Position(33545, 31016, 15)},
	[3] = {fromPos = Position(33518, 31055, 15), toPos = Position(33545, 31016, 15)},
	[4] = {fromPos = Position(33519, 31055, 15), toPos = Position(33545, 31016, 15)},
	[5] = {fromPos = Position(33520, 31055, 15), toPos = Position(33545, 31016, 15)}
}


local configg = {
    maxPlayers = 0,
    room = {fromPos = Position(33541, 31009, 15), toPos = Position(33549, 31016, 15)},
    
}

function LordRetroxArea()
   Game.createMonster("Lord Retro", Position(33545, 31010, 15))
 
	return	
end

local LordRetro = Action()
function LordRetro.onUse(player, item, fromPosition, target, toPosition, isHotkey)
...
if #getPlayersInArea(configg.room.fromPos, configg.room.toPos) > configg.maxPlayers then
  ...
  if player:getStorageValue(config.timeStorage) > os.time() then
	...
	addEvent(LordRetroxArea, 500)
    clearRoom(Position(33542, 31008, 15), Position(33550, 31016, 15), true)
    return true
end
