...
local configg = {
    maxPlayers = 0,
    room = {fromPos = Position(32952, 32677, 2), toPos = Position(32958, 32682, 2)},
    
}

function TheFlamingOrchidxArea()
   Game.createMonster("The Flaming Orchid", Position(32953, 32679, 2))
   
    return 
end

local TheFlamingOrchid = MoveEvent()

function TheFlamingOrchid.onStepIn(creature, item, position, fromPosition)
...
if #getPlayersInArea(configg.room.fromPos, configg.room.toPos) > configg.maxPlayers then
  ...
  ---------------------///// parate para agregar a otra quest-----------------------
	...
	player:teleportTo(Position(32958, 32679, 2))
  ...
  addEvent(TheFlamingOrchidxArea, 500)
    clearRoom(Position(32952, 32677, 2), Position(32958, 32682, 2), true)

    return true
end
	
TheFlamingOrchid:type("stepin")
TheFlamingOrchid:aid(64116)
TheFlamingOrchid:register()
