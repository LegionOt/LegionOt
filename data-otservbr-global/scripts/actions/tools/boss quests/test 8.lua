local config = {
    ...
    timeStorage = 716, -- id of lever before pulled
...
local player_positions = {
    [1] = {fromPos = Position(33669, 32926, 15), toPos = Position(33651, 32906, 15)},
	[2] = {fromPos = Position(33669, 32927, 15), toPos = Position(33651, 32906, 15)},
	[3] = {fromPos = Position(33669, 32928, 15), toPos = Position(33651, 32906, 15)},
	[4] = {fromPos = Position(33669, 32929, 15), toPos = Position(33651, 32906, 15)},
	[5] = {fromPos = Position(33669, 32930, 15), toPos = Position(33651, 32906, 15)}
}


local configg = {
    maxPlayers = 0,
    room = {fromPos = Position(33645, 32900, 15), toPos = Position(33660, 32915, 15)},
    
}

function test8xArea()
   Game.createMonster("Magma Bubble", Position(33654, 32912, 15))  
 
	return	
end

local test8 = Action()
function test8.onUse(player, item, fromPosition, target, toPosition, isHotkey)
...
if #getPlayersInArea(configg.room.fromPos, configg.room.toPos) > configg.maxPlayers then
  ...
  if player:getStorageValue(config.timeStorage) > os.time() then
	...
	addEvent(test8xArea, 500)
	clearRoom(Position(33644, 32899, 15), Position(33663, 32920, 15), true)
    return true
end
