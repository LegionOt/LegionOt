...
local function resetBossStorageBugado(cid)
	local player = Player(cid)
	if not player then return false end
	
	local desbuggedStr = 30164
	
	if player:getStorageValue(desbuggedStr) > 0 then
		return false
	end
	local storagesBug = {
		Storage.WereLionMiniBosses.YirkasBlueScales,
		Storage.WereLionMiniBosses.KatexBloodTongue,
		Storage.WereLionMiniBosses.SrezzYellowEyes,
		Storage.WereLionMiniBosses.UtuaStoneSting,
		Storage.WereWolfMiniBosses.BloodBackTimer,
		Storage.WereWolfMiniBosses.DarkFangTimer,
		Storage.WereWolfMiniBosses.SharpclawTimer,
		Storage.WereWolfMiniBosses.ShadowpeltTimer,
		Storage.WereWolfMiniBosses.BlackVixenTimer,
		Storage.FeasterOfSouls.UnazTheMeanTimer,
		Storage.FeasterOfSouls.IrgixTheFlimsyTimer,
		Storage.FeasterOfSouls.VokTheFreakish,
	}
	
	for _, str in pairs(storagesBug) do
		if player:getStorageValue(str) > 0 then
			player:setStorageValue(str, 0)
		end
	end
	
	player:setStorageValue(desbuggedStr, 1)
	return true
end

local function add_event_vip(cid)
	local player = Player(cid)
	if not player then return false end
	if player:getStorageValue(Storage.vip_event_add) <= 0 then
		player:addVipDays(3)
		player:setStorageValue(Storage.vip_event_add, 1)
		player:sendTextMessage(MESSAGE_LOGIN, "Você recebeu 3 dias de VIP!")
	end 
 	return true
end
...
function playerLogin.onLogin(player)
	add_event_vip(player:getId())
	resetBossStorageBugado(player:getId())
	-- init autolootEnabled storage
	checkDromeCharmUpgradeBonus(player:getId())
	...
	-- Evento Double Exp
	if Game.getStorageValue(Storage.DoubleExp) - os.time() >= 0 then
		player:sendTextMessage(19, "[Event]: Double EXP activated per ".. getStorageTime(Storage.DoubleExp) ..".")
		doubleExp = true
	end
	...
	player:setDisplayExp()

	if player:getStorageValue(23123) >= 1 then --Reset exercise weapon storage
		player:setStorageValue(23123, 0)
	end
	
	-- local resultName = db.storeQuery("SELECT `tournamentchek` FROM `accounts` WHERE (`id` = " .. player:getAccountId() .. ")")
	-- local coins = result.getDataInt(resultName, "tournamentchek")
	-- result.free(resultName)

	-- if coins == 0 then
		-- db.query("UPDATE `accounts` SET `tournamentchek` = '1' WHERE `id` = ".. player:getAccountId() .."")
		-- player:addTournamentCoinsBalance(1000, true)
		-- db.query("UPDATE `accounts` SET `coins` = (`coins`+ " .. 1000 .. ") WHERE `id` = " .. player:getAccountId())
		-- player:setStorageValue(Storage.firstCoinsBeta, 1)
	-- end
	sendItemClasses(player:getGuid())
	return true
end
